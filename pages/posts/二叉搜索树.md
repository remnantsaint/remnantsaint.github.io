...
### 递归
#### 二叉树
如果不是二叉搜索树，而是普通的二叉树，那么就遍历树，然后用 map 统计频率，最后把频率排个序取前面的高频元素集合（使用了额外空间）  
代码如下：
```c++
class Solution {
private:
...
```
#### 二叉搜索树
如果是二叉搜索树的话，中序是有序的，用相邻两个元素作比较，不需要额外空间  
弄一个指针指向前一个结点，这样每次 cur （当前结点） 才能和 pre（前一个结点）作比较  
初始化 pre = NULL ，这样当 pre == nullptr 的时候，就知道这时比较的第一个元素  
代码如下：  
```c++
class Solution {
private:
